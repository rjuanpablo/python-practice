...
def suma_cubo_pares_for(numeros: Iterable[int]) -> int:
    ...
    for numero in numeros:
        numero = numero ** 3
        if numero % 2 == 0:
            numeros_cuadrados.append(numero)
    for numero in numeros_cuadrados:
        total = total + numero
...
def suma_cubo_pares_sum_list(numeros: Iterable[int]) -> int:
    ...
    return sum(numero ** 3 for numero in numeros if (numero ** 2) % 2 == 0)
...
suma_numeros_al_cubo_pares = reduce(lambda acumulador, x: acumulador + (x ** 3 if x % 2 == 0 else 0), numeros,0)


# Escribir una función Lambda que permita ordenar los elementos de la numeros
# en base a si son pares o impares

numeros_ordenada = sorted(numeros, key=lambda x: (x % 2 != 1, x))
